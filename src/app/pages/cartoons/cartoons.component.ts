...
import { Genre, ContentId } from '@models/core';
...
export class CartoonsPageComponent implements OnInit, OnDestroy {
  private activatedRouteSubscriber: Subscription;
  private backServiceSubscriber: Subscription;
  private readonly categoryId = 3; // Cartoons
  ...
  public loadVideos(): void {
    this.dataService.getVideoContent(this.categoryId, this.activeGenre.id, this.size, this.skip).then((response) => {
